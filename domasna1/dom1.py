...
def scrape_info(row, issuer):
    turnoverBEST = row.select_one('td:nth-child(8)').text.strip()
    totalTurnover = row.select_one('td:nth-child(9)').text.strip()
    row_dict = {}
    if (totalTurnover != "0" and turnoverBEST != "0"):
        ...
        lastTrade = row.select_one('td:nth-child(2)').text.strip()
        ...
        row_dict = {
            "Shifra": issuer,
            "Datum": datum,
            "Cena na posledna transakcija": lastTrade,
            "Mak.": max,
            "Min.": min,
            "Prosecna cena": avg,
            "%prom": prom,
            "Kolicina": kol,
            "Promet vo BEST vo denari": turnoverBEST,
            "Vkupen promet": totalTurnover
        }
    return row_dict
...
def fill_data(issuers):
    ...
    for issuer in issuers:
        ...
        url_t = url_base + issuer + "?fromDate=1.1." + today_values[0] + "&toDate=" + today_values[2] + "." + \
